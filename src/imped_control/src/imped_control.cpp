                             ...
                             ros::NodeHandle &n)
{
	...
	{
		ROS_ERROR("ImpedControlClass could not use the chain from '%s' to '%s'",
	...
	{
		ROS_ERROR
		("ImpedControlClass could not use the chain from '%s' to '%s'",
		 root_name.c_str(), tip_name.c_str());
		return false;
	}
	...
	Mass_d *= 5;
	Damp_d = Eigen::MatrixXd::Identity(6, 6);
	Damp_d *= 300;
...
void ImpedControlClass::starting()
{
	...
	for (int i = 0; i < Joints; i++)
	{
		...
		joint_residual[i].set_gravity(G_.data(i));
	}
...
void ImpedControlClass::update()
{
	...
	for (int i = 0; i < Joints; i++)
	{
		joint_residual[i].set_tau(tau_task_e(i));
		joint_residual[i].set_gravity(G_.data(i));
